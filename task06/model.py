...
class Conditional(ASTNode):
    ...
    вычисляется if_f    alse список, иначе if_true.
    ...
    def evaluate(self, scope):
        statements = self.if_true if (
                self.condition.evaluate(scope) != Number(0)
        ) else self.if_false
        res = None
...
class FunctionCall(ASTNode):
    ...
    def evaluate(self, scope):
        ...
        for arg_name, arg_value_expr in zip(function.args, self.args):
            call_scope[arg_name] = arg_value_expr.evaluate(scope)
        res = None
